...
{
    public class PrincipalBalance : IBalance
    {
        ...
        public decimal Balance(string BillingType, string BillingReference, int CompanyId, int MaxBillingPeriod)
        {
            ...
            var hasForwarded = db.Balances.Where(m => m.BillingType == BillingType)
                ...
                .Where(m => m.BillingReference == BillingReference)
            ...
            if (hasForwarded.Count > 0)
            {
                ...
                //var balCr1 = db.Balances.Where(m => m.BillingType == BillingType).Where(m => m.BillingSubType == "BALANCE")
                ...
                var slDr1 = db.SubsidiaryLedger.Where(m => m.BillingType == BillingType).Where(m => m.BillingSubType == "PRINCIPAL").Where(m => m.BillingReference == BillingReference).Where(m => m.CompanyId == CompanyId).Where(m => m.BillingPeriod == MaxBillingPeriod).Sum(m => (decimal?)m.DebitAmount) ?? 0;
                var slCr1 = db.SubsidiaryLedger.Where(m => m.BillingType == BillingType).Where(m => m.BillingSubType == "PRINCIPAL").Where(m => m.BillingReference == BillingReference).Where(m => m.CompanyId == CompanyId).Where(m => m.BillingPeriod == MaxBillingPeriod).Sum(m => (decimal?)m.CreditAmount) ?? 0;
                var slCrWTAX = db.SubsidiaryLedger.Where(m => m.BillingType == BillingType).Where(m => m.BillingSubType == "WTAX").Where(m => m.BillingReference == BillingReference).Where(m => m.CompanyId == CompanyId).Where(m => m.BillingPeriod == MaxBillingPeriod).Sum(m => (decimal?)m.CreditAmount) ?? 0;
                slCr1 += slCrWTAX;
                bool isCancel = false;
                try
                { //If has Cancel OR Payment. Do not Subtract CR in SL
                    ...
                    var maxId = db.SubsidiaryLedger.Where(m => m.BillingType == BillingType).Where(m => m.TransactionType.ToUpper() == "PAYMENT").Where(m => m.BillingSubType == "PRINCIPAL").Where(m => m.BillingReference == BillingReference).Where(m => m.CompanyId == CompanyId).Where(m => m.BillingPeriod == MaxBillingPeriod).Max(x => x.SubsidiaryLedgerId);
