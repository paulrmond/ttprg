...
{
    public class PrincipalBalanceWaterAndSewerage : IBalance
    {
        public decimal Balance(string BillingType, string BillingReference, int CompanyId, int MaxBillingPeriod)
        {
            ...
            if (hasForwarded.Count > 0)
            {
                ...
                var slDr1 = db.SubsidiaryLedger.Where(m => m.BillingType == BillingType).Where(m => m.BillingReference == BillingReference).Where(m => m.BillingSubType == "PRINCIPAL").Where(m => m.CompanyId == CompanyId).Where(m => m.BillingPeriod == MaxBillingPeriod).Sum(m => (decimal?)m.DebitAmount) ?? 0;
                var slCr1 = db.SubsidiaryLedger.Where(m => m.BillingType == BillingType).Where(m => m.BillingReference == BillingReference).Where(m => m.BillingSubType == "PRINCIPAL").Where(m => m.CompanyId == CompanyId).Where(m => m.BillingPeriod == MaxBillingPeriod).Sum(m => (decimal?)m.CreditAmount) ?? 0;
                var slCrWTAX = db.SubsidiaryLedger.Where(m => m.BillingType == BillingType).Where(m => m.BillingReference == BillingReference).Where(m => m.BillingSubType == "WTAX").Where(m => m.CompanyId == CompanyId).Where(m => m.BillingPeriod == MaxBillingPeriod).Sum(m => (decimal?)m.CreditAmount) ?? 0;
                slCr1 += slCrWTAX;
                ...
                { //If has Cancel OR Payment. Do not Subtract CR in SL
                  //Add this .Where(m => m.TransactionType.ToUpper() == "PAYMENT") and .Max(x => x.SubsidiaryLedgerId)
                    ...
                    var maxId = db.SubsidiaryLedger.Where(m => m.BillingType == BillingType).Where(m => m.BillingReference == BillingReference).Where(m => m.TransactionType.ToUpper() == "PAYMENT").Where(m => m.BillingSubType == "PRINCIPAL").Where(m => m.CompanyId == CompanyId).Where(m => m.BillingPeriod == MaxBillingPeriod).Max(x => x.SubsidiaryLedgerId);
