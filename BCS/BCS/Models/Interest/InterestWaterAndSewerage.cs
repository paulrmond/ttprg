...
{
    public class InterestWaterAndSewerage : IInterestWithBP
    {
        public decimal Interest(string BillingType, int CompanyId, string billingReference, DateTime CoverageFrom, DateTime BillDate, decimal amount, string LastId, decimal currentPrincipalAmount)
        {
            ...
            var a = db.SubsidiaryLedger
                .Where(m => m.CompanyId == CompanyId && m.BillingType.ToUpper() == BillingType.ToUpper() && m.BillingReference == billingReference && m.TransactionType.ToUpper() == "BILLING" && m.BillingPeriod == db.SubsidiaryLedger.Where(k => k.CompanyId == CompanyId && k.BillingType == BillingType && k.TransactionType == "BILLING").Max(j => j.BillingPeriod))
